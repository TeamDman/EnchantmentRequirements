...
public class ClientStuff {
	...
	private static void handleItemPower(ItemTooltipEvent event, ItemStack stack, ClientLevel level, BlockPos pos, List<Component> tip) {
		int enchantValue = stack.getItemEnchantability();
		if (enchantValue < Config.MIN_ENCHANTMENT_VALUE_TO_SHOW.get()) return;
		tip.add(new TranslatableComponent("tooltip.enchantmentrequirements.value", enchantValue));
	}
	...
	private static void handleBlockPower(ItemTooltipEvent event, ItemStack stack, ClientLevel level, BlockPos pos, List<Component> tip) {
		...
		if (power > 0) {
			tip.add(new TranslatableComponent("tooltip.enchantmentrequirements.power", power));
		}
	}
	...
	private static void handleTablePower(ItemTooltipEvent event, ItemStack stack, Level level, BlockPos pos, List<Component> tip) {
		...
		tip.add(new TranslatableComponent("tooltip.enchantmentrequirements.table_power", tablePower).withStyle(ChatFormatting.DARK_AQUA));
		int itemPower = stack.getItemEnchantability();
		...
		tip.add(new TranslatableComponent("tooltip.enchantmentrequirements.cost_0", cost0Lower, cost0Upper).withStyle(ChatFormatting.DARK_AQUA));
		tip.add(new TranslatableComponent("tooltip.enchantmentrequirements.cost_1", cost1Lower, cost1Upper).withStyle(ChatFormatting.DARK_AQUA));
		tip.add(new TranslatableComponent("tooltip.enchantmentrequirements.cost_2", cost2Lower, cost2Upper).withStyle(ChatFormatting.DARK_AQUA));
	}
	...
	private static void handleBookLevel(ItemTooltipEvent event, ItemStack stack, ClientLevel level, BlockPos pos, List<Component> tip) {
		...
		tip.add(new TranslatableComponent("tooltip.enchantmentrequirements.cost", minCost, maxCost));
		...
		var itemPower = enchanting.getItemEnchantability();
		/**
		 * Do the math from {@link EnchantmentHelper#getEnchantmentCost(RandomSource, int, int, ItemStack)}
		 */
		if (itemPower <= 0) return;
		...
		tip.add(new TranslatableComponent("tooltip.enchantmentrequirements.true_cost", pLevelLower, pLevelUpper).withStyle(ChatFormatting.DARK_AQUA));

		if (!ench.canEnchant(enchanting) && !((enchanting.getItem() instanceof BookItem) && ench.isAllowedOnBooks())) {
			tip.add(new TranslatableComponent("tooltip.enchantmentrequirements.cant_enchant").withStyle(ChatFormatting.RED));
		}
	}
