...
public class ClientStuff {
	...
	private static void handleBookLevel(ItemTooltipEvent event, ItemStack stack, ClientLevel level, BlockPos pos, List<Component> tip) {
		...
		tip.add(Component.translatable("tooltip.enchantmentrequirements.true_cost", pLevelLower, pLevelUpper)
						.withStyle(ChatFormatting.DARK_AQUA));

		if (!ench.canEnchant(enchanting) && !((enchanting.getItem() instanceof BookItem) && ench.isAllowedOnBooks())) {
			tip.add(Component.translatable("tooltip.enchantmentrequirements.cant_enchant")
